...
export default function AddBranchInfoPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchClients(), fetchStates()])
      .finally(() => setLoading(false));
  }, [router, fetchClients, fetchStates]);

  // Auto-select EXEMPTED when client_type is SEZ
  useEffect(() => {
    if (formData.client_type === 'SEZ') {
      setFormData(prev => ({ ...prev, sc_i: 'EXEMPTED' }));
    } else if (formData.client_type === '' && formData.sc_i === 'EXEMPTED') {
      setFormData(prev => ({ ...prev, sc_i: '' }));
    }
  ...
  const handleSubmit = async (saveAndNext = false) => {
    ...
    setSubmitting(true);
    try {
      const response = await api.post('/client-bu', formData);
      success('Branch info created successfully');
      
      if (saveAndNext) {
        // Navigate to Service Charges form
        router.push(`/dashboard/control-room/client/add-service-charges?clientId=${formData.client_info_id}&buId=${response.data.id}`);
      } else {
        router.push('/dashboard/control-room/client');
      }
    } catch (err) {
      console.error('Error creating branch info:', err);
      showError(err.response?.data?.error || 'Failed to create branch info');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <>
      ...
      <div className="mb-6">
        <div className="flex items-center mb-2">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Add Branch Info</h1>
        </div>
        <p className="text-sm text-gray-600 ml-10">Create a new branch/division record</p>
      </div>
