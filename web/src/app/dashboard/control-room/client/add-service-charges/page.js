...
export default function AddServiceChargesPage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const preSelectedClientId = searchParams.get('clientId');
  const preSelectedBuId = searchParams.get('buId');
  ...
  const [formData, setFormData] = useState({
    account_id: '',
    client_info_id: preSelectedClientId || '',
    client_bu_id: preSelectedBuId || '',
    job_register_id: '',
    concern_person: '',
    concern_email_id: '',
    concern_phone_no: '',
  ...
  const fetchClients = useCallback(async (accountId = null) => {
    try {
      let url = '/client-info';
      if (accountId) {
        url = `/client-info/by-account/${accountId}`;
      }
  ...
  const fetchNextGroupId = useCallback(async () => {
    try {
      const response = await api.get('/client-service-charges/next-group-id');
      setNextGroupId(response.data.group_id);
    } catch (err) {
      console.error('Error fetching next group ID:', err);
      showError(err.response?.data?.error || 'Failed to fetch next group ID');
    }
  }, [showError]);
  ...
  }, [router, fetchAccounts, fetchClients, fetchJobRegisters, fetchNextGroupId]);
  ...
  }, [formData.client_info_id, fetchBranches]);

  // Update clients when account changes
  useEffect(() => {
    if (formData.account_id) {
      fetchClients(formData.account_id);
      // Reset client and branch selection when account changes
      setFormData(prev => ({
        ...prev,
        client_info_id: '',
        client_bu_id: '',
  ...
  }, [formData.account_id, fetchClients]);
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await api.post('/client-service-charges', {
        ...formData,
        group_id: nextGroupId,
      });
      success('Service charges created successfully');
      router.push('/dashboard/control-room/client');
    } catch (err) {
      console.error('Error creating service charges:', err);
      showError(err.response?.data?.error || 'Failed to create service charges');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <>
      ...
      <div className="mb-6">
        <div className="flex items-center mb-2">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Add Service Charges</h1>
        </div>
        <p className="text-sm text-gray-600 ml-10">Create a new service charges record</p>
      </div>
