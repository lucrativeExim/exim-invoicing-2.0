...
export default function JobPage() {
  ...
  const fetchJobs = async () => {
    try {
      setLoading(true);
      // Fetch jobs with status='In_process' and job_id_status='Active'
      const response = await api.get('/jobs?status=In_process&jobIdStatus=Active');
  ...
  const handleClosedJob = () => {
    router.push('/dashboard/job/job/closed');
  };
