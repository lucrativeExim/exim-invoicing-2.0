...
const Input = ({
  ...
  // If type is 'date', use DateInput component instead
  if (type === 'date') {
    return (
      <DateInput
        label={label}
  ...
  return (
    <div className={`mb-2 ${className}`}>
      ...
      <div className="relative">
        <input
          type={inputType}
          className={`input-field ${error ? 'border-red-500 focus:ring-red-500' : ''} ${isPasswordField ? 'pr-10' : ''} ${props.disabled ? 'bg-gray-100 cursor-not-allowed' : ''}`}
          id={name}
          name={name}
          value={value || ''}
          onChange={onChange}
